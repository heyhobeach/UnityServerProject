...
namespace UnityServerProject.Model
{
    ...
    1.1 StageDeathInfo 같은 경우는 stageid 같은 내용처럼 stage를 구별하는 id가 필요함(완료)
    1.2 DeathInfo의 경우는  지금 처럼 DeathId로 진행해야함(완료)

    2. DeathInfos의 경우 EFCore의 경우 자동으로 리스트는 외래키를 생성해줌 따라서 StageDeathInfo의 리스트는 유지(진행중)
    3. DeathInfo의 경우 Vector2는 직접 지정해줄 필요 있음 따라서 DeathPositionX DeathPositionY EnemyPositionX EnemyPositionY를 float 형태로 해서 정의 해야함(완료)
    4. 클라이언트에서 vector2로 전송하는건 그대로 하되 서버에서 json을 받아서 분해해서 저장하는식으로 접근해야함(아직안함)
    5. 테이블 추가 생성 이유 현재는 테이블이 userplaydata밖에없기때문에 다른 테이블을 생성해서 연결해야함
    */
